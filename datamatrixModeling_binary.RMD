...
output: pdf_document
params:
  csvPath:            datalocation/pyradiomics_modeling.csv
  target:             MutationalStatus
  positive_class:     mut
  inputs:            !r NULL 
  leaveOneOut:       !r TRUE
  test_csv:           datalocation/pyradiomicsout_experimentalcohort.csv
  rescale:           !r FALSE
...
TODO: add order="hclust" to corrplot calls
...
dsraw <- datamatrix[ c(params$target, allinputs) ]
...
```{r Performing Variable Selection, echo=FALSE, warning=FALSE, fig.width=7.5, fig.height=8}
...
boxplot( as.formula( paste(variableSelections$boruta[[iii]], "~", params$target) ),
 ...
 cat('WT summary\n')
 print(summary(borutavariable[which(ds$MutationalStatus == 'WT'),]))
 cat('\n')
 cat('mut summary\n')
 print(summary(borutavariable[which(ds$MutationalStatus == 'mut'),]))
 cat('\n')
}# end boxplots
} #end if >0 variables
